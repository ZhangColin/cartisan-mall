...
CREATE TABLE `mbr_member_levels` (
    ...
    `growth_point` int NOT NULL COMMENT '等级需要的成长值',
    `privilege_free_freight` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否有免邮特权',
    `free_freight_point` int NOT NULL DEFAULT 0 COMMENT '免运费标准',
    `privilege_comment` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否有评论获取奖励特权',
    ...
    `privilege_sign_in` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否有签到特权',
    `privilege_promotion` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否有专享活动特权',
