...
CREATE TABLE `mall_category` (
                                 ...
                                 `parent_id` int(11) COMMENT '父类别id当id=0时说明是根节点,一级类别',
                                 `name` varchar(50) COMMENT '类别名称',
                                 `status` tinyint(1) DEFAULT '1' COMMENT '类别状态1-正常,2-已废弃',
                                 `sort_order` int(4) COMMENT '排序编号,同类展示顺序,数值相等则自然排序',
...
INSERT INTO `mall_category` (`id`, `parent_id`, `name`, `status`, `sort_order`, `created`, `updated`)
VALUES
...
CREATE TABLE `mall_order` (
                              ...
                              `postage` int(10) COMMENT '运费,单位是元',
                              `status` int(10) COMMENT '订单状态:0-已取消-10-未付款，20-已付款，40-已发货，50-交易成功，60-交易关闭',
...
CREATE TABLE `mall_order_item` (
                                   ...
                                   `product_image` varchar(500) COMMENT '商品图片地址',
                                   `current_unit_price` decimal(20,2) COMMENT '生成订单时的商品单价，单位是元,保留两位小数',
                                   `quantity` int(10) COMMENT '商品数量',
                                   `total_price` decimal(20,2) COMMENT '商品总价,单位是元,保留两位小数',
                                   `created` datetime,
                                   `updated` datetime,
                                   PRIMARY KEY (`id`),
...
CREATE TABLE `mall_product` (
                                ...
                                `main_image` varchar(500) COMMENT '产品主图,url相对地址',
...
INSERT INTO `mall_product` (`id`, `category_id`, `name`, `subtitle`, `main_image`, `sub_images`, `detail`, `price`, `stock`, `status`, `created`, `updated`)
VALUES
...
CREATE TABLE `mall_shipping` (
                                 `id` int(11) NOT NULL AUTO_INCREMENT,
                                 `user_id` int(11) COMMENT '用户id',
                                 `receiver_name` varchar(20) COMMENT '收货姓名',
                                 `receiver_phone` varchar(20) COMMENT '收货固定电话',
                                 `receiver_mobile` varchar(20) COMMENT '收货移动电话',
                                 `receiver_province` varchar(20) COMMENT '省份',
...
INSERT INTO `mall_shipping` (`id`, `user_id`, `receiver_name`, `receiver_phone`, `receiver_mobile`, `receiver_province`, `receiver_city`, `receiver_district`, `receiver_address`, `receiver_zip`, `created`, `updated`)
VALUES
...
CREATE TABLE `mall_user` (
                             ...
                             `email` varchar(50),
                             `phone` varchar(20),
                             `question` varchar(100) COMMENT '找回密码问题',
                             `answer` varchar(100) COMMENT '找回密码答案',
                             `role` int(4) NOT NULL COMMENT '角色0-管理员,1-普通用户',
                             `created` datetime NOT NULL COMMENT '创建时间',
                             `updated` datetime NOT NULL COMMENT '最后一次更新时间',
                             PRIMARY KEY (`id`),
                             UNIQUE KEY `user_name_unique` (`username`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO `mall_user` (`id`, `username`, `password`, `email`, `phone`, `question`, `answer`, `role`, `created`, `updated`)
VALUES
...
CREATE TABLE `mall_pay_info` (
                                 ...
                                 `pay_platform` int(10) COMMENT '支付平台:1-支付宝,2-微信',
                                 `platform_number` varchar(200) COMMENT '支付流水号',
                                 `platform_status` varchar(20) COMMENT '支付状态',
...
alter table mall_user modify created datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间';
alter table mall_user modify updated datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后一次更新时间';
