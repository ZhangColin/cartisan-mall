...
CREATE TABLE `mkt_member_prices` (
    ...
    INDEX `index_member_price_product_id_sku_id`(`product_id`, `sku_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商品会员价';

-- 商品满减表
DROP TABLE IF EXISTS `mkt_product_full_reductions`;

CREATE TABLE `mkt_product_full_reductions` (
    `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'ID',
    `product_id` bigint NOT NULL COMMENT '商品ID',
    ...
    `updated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`) USING BTREE,
    INDEX `index_product_full_reduction_product_id`(`product_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商品满减';

-- 商品阶梯价表
DROP TABLE IF EXISTS `mkt_product_ladders`;

CREATE TABLE `mkt_product_ladders` (
    `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'ID',
    `product_id` bigint NOT NULL COMMENT '商品ID',
    `count` int NOT NULL COMMENT '满几件',
    ...
    `updated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`) USING BTREE,
    INDEX `index_product_ladder_product_id`(`product_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商品阶梯价';
